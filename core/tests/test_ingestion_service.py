...
class TestIngestionService(BaseLyraTestCase):

    def test_ingestion_service_cpp(self):
        """
        Verify the C++ IngestionService implementation.
        """
        executable = "./core/build/ingestion_service_test"
        ...
        # We expect the return code to be 0 (ALL_INGESTION_SERVICE_TESTS_PASSED)
        self.assertEqual(res.returncode, 0, f"Expected exit code 0, got {res.returncode}.\nSTDOUT: {res.stdout}\nSTDERR: {res.stderr}")
        self.assertIn("ALL_INGESTION_SERVICE_TESTS_PASSED", res.stdout)
