...
class TestLyraCore(unittest.TestCase):
    ...
    def test_artist_create_missing_required(self):
        """Test missing required parameters (id or name): JsonValidator should return 400"""
        res = self.dispatch("CreateArtist", {"name": "Missing ID Artist"})
        self.assertEqual(res["code"], 400)
        self.assertEqual(res["error"]["type"], "MissingParameter")

        res2 = self.dispatch("CreateArtist", {"id": str(uuid.uuid4())})
        self.assertEqual(res2["code"], 400)
        self.assertEqual(res2["error"]["type"], "MissingParameter")
