...
class IPlaylistRepository {
  public:
    ...
    virtual tl::expected<void, std::string> insert(
        const Playlist &playlist) = 0;
    virtual tl::expected<void, std::string> update(
        const PlaylistUpdate &update_data) = 0;
    virtual tl::expected<Playlist, std::string> get(
    ...
    virtual tl::expected<void, std::string> add_track(
    ...
    virtual tl::expected<std::string, std::string> get_first_track_id(
        const std::string &playlist_id) = 0;
    virtual tl::expected<std::vector<Playlist>, std::string> get_by_title(
