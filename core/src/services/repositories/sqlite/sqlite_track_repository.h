...
class SqliteTrackRepository : public ITrackRepository, public SqliteEntityRepository<Track, TrackUpdate> {
  public:
    ...
    tl::expected<std::vector<Track>, std::string> get_by_pcm_hash(const std::string &pcm_hash) override;

  protected:
    tl::expected<void, std::string> do_insert(SQLite::Database &db, const Track &track) override;
    void build_update(SqliteUpdateBuilder &builder, const TrackUpdate &data) const override;
};
